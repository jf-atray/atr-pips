...
use glam::prelude::Vec3Swizzles;
use glam::{Vec2, Vec3};
use rand::Rng;

use crate::arena::makers::ProjectileBlueprint;
use crate::arena::tables::{ActorAddition, PilotState, Team};
use crate::brushes::Brush;
use crate::query::impls::{query_mut_mut_mut_mut, query_ref_ref};
...
impl Script for PilotScript {
    fn update(&mut self, ctx: &mut DomainView) {
        ...
        {
            let (_ids, tables) = ctx.split();
            let TablesView { core, system, mut additions } = tables.view();
            let xforms = &mut core.xforms;
            let motions = &mut core.motions;
            let (_team_map, enemy_positions) = {
            ...
            for (pilot_col, motion_col, team_col, xform_col) in query_mut_mut_mut_mut(
                &mut actor.pilot,
                &(),
                motions,
                &(),
                ...
                for i in 0..pilot_col.len() {
                    ...
                    match &mut pilot_data.state {
                        PilotState::Wander { goal, timer } => {
                            *timer -= dt;
                            let to_goal = *goal - xform;

                            if is_player && !enemy_positions.is_empty() {
                                let threat = enemy_positions
                                    .iter()
                                    .fold(Vec2::ZERO, |acc, &e| acc + (xform - e).normalize_or_zero());
                                ...
                                *timer = 3.0;
                                let dir = (*goal - xform).normalize_or_zero();
                                motion.vel = Vec3::new(dir.x, dir.y, 0.0) * pilot_data.speed;
                            } else {
                        ...
                        PilotState::Chase { target: _ } => {
                            // RED FLAG HERE - we've got a two-body constraint woven into the solver.
                            todo!();
                            /*let Some(target_xform) = gather_ref(ids, xform, *target) else {
                                pilot_data.state = PilotState::Wander { goal: xform, timer: 1.0 };
                                continue;
                            };
                            let target_pos = target_xform.xyz.truncate();
                            let dir = (target_pos - xform).normalize_or_zero();
                            motion.vel = Vec3::new(dir.x, dir.y, 0.0) * pilot_data.speed;

                            if pilot_data.cooldown <= 0.0 && dir != Vec2::ZERO {
                                let spawn_pos = xform + dir * 0.8;
