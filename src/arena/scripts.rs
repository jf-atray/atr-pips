...
use crate::arena::tables::{PilotAddition, PilotState, ProjectileData};
...
const FIRE_COOLDOWN: f32 = 0.45;
const WANDER_BOUNDS: f32 = 15.0;
...
impl Script for PilotScript {
    fn update(&mut self, ctx: &mut DomainView) {
...

        {
            ...
            let pilot = {
                let Some(boxed) = tables.additions.get_mut(&std::any::TypeId::of::<PilotAddition>()) else {
                    return;
                };
                    ...
                    match &mut pilot_data.state {
                        PilotState::Wander { goal, timer } => {
                            *timer -= dt;
                            ...
                            if to_goal.length() < 0.5 || *timer <= 0.0 {
                                *goal = Vec2::new(
